...
export function ClientTimelinePage() {
  const { projects, activeProjectId, setActiveProjectId } = useAuth();

  const selected = projects.find(p => p._id === activeProjectId) || projects[0] || null;
  ...
  const stageBadge = (s) => {
    ...
    if (s === "delayed") return { icon: "error", color: "#d32f2f" };
                ...
                <Card className="p-6">
                  ...
                  <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: CS.surfaceLow }}>
                    <div className="h-full rounded-full transition-all duration-700 relative overflow-hidden"
                      ...
                      <div className="absolute inset-0 bg-[#f0ede4]/20 animate-pulse" />
                    </div>
                  </div>
                </Card>
                        ...
                        return (
                          <div key={i} className="relative flex gap-5 items-start">
                            <div className={`z-10 w-6 h-6 rounded-full flex items-center justify-center ring-4 ring-white flex-shrink-0`}
                              style={{
                                background: stage.status === "completed" ? CS.primaryContainer : stage.status === "in_progress" ? CS.primaryFixed : stage.status === "delayed" ? "#ffebee" : CS.surfaceContainer,
                              }}>
                              <span className="material-symbols-outlined text-[14px]" style={{ color: sb.color }}>
                                {stage.status === "completed" ? "check" : stage.status === "in_progress" ? "radio_button_checked" : stage.status === "delayed" ? "error" : "schedule"}
                              </span>
                            </div>
...
export function ClientMeetingsPage() {
  ...
  const filteredMeetings = meetings.filter(m => !m.projectId || m.projectId === activeProjectId);

  useEffect(() => {
    if (filteredMeetings.length > 0) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setSelected(filteredMeetings.find(x => x.status === "confirmed") || filteredMeetings[0] || null);
    } else {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setSelected(null);
    }
  ...
  async function handleRequest(e) {
    ...
    setError("");
    try {
      const payload = { ...form, projectId: activeProjectId || undefined };
      const m = await clientApi.requestMeeting(payload, token);
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border"
                          style={{ background: CS.surfaceLow, borderColor: CS.outlineVariant }}>
                          <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-[#f0ede4]"
...
export function ClientDocumentsPage() {
  ...
  const filtered = docs.filter(d => {
    const matchesProject = !d.projectId || d.projectId === activeProjectId;
...
export function ClientBillingPage() {
  const { token, projects, activeProjectId } = useAuth();
  ...
  useEffect(() => {
    clientApi.getOrders(token).then(o => {
      setOrders(o);
      const matched = o.find(x => x._id === activeProject?.orderId);
      setSelectedOrder(matched || o[0] || null);
    }).catch(() => {}).finally(() => setLoading(false));
  }, [token, activeProject?.orderId]);
  ...
  async function downloadInvoicePdf(order) {
    ...
    const invoiceId = order.payment?.invoiceId || order.id || "—";
    const dateVal = order.payment?.paidAt || order.createdAt;
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text(`Invoice No: ${invoiceId}`, pageWidth - margin - 180, 72);
    doc.text(`Date: ${dateStr}`, pageWidth - margin - 180, 87);
    ...
    doc.setFont("helvetica", "bold");
    doc.text(order.customer?.customerName || "Client", pageWidth / 2 + 20, y + 18);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.setTextColor(75, 85, 99);
    
    const company = order.customer?.company || "Individual Client";
    const phone = order.customer?.phone || "";
    const email = order.customer?.customerEmail || "";
    const gstin = order.customer?.gstin ? `GSTIN: ${order.customer.gstin}` : "";
    ...
    if (order.customer?.billingAddress && (order.customer.billingAddress.line1 || order.customer.billingAddress.city)) {
    ...
    const basePrice = order.package?.price || 0;
    const total = order.package?.total || Math.round(basePrice * 1.18);
    const gst = total - basePrice;

    // Table Row
    doc.setFont("helvetica", "normal");
    doc.setTextColor(17, 24, 39);
    doc.text(order.package?.name || "Standard Package", margin + 15, y + 25);
    doc.text(`₹${basePrice.toLocaleString("en-IN")}`, pageWidth - margin - 260, y + 25, { align: "right" });
    doc.text(`₹${gst.toLocaleString("en-IN")}`, pageWidth - margin - 130, y + 25, { align: "right" });
    doc.setFont("helvetica", "bold");
    doc.text(`₹${total.toLocaleString("en-IN")}`, pageWidth - margin - 15, y + 25, { align: "right" });
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(17, 24, 39);
    doc.text(`Status: ${order.payment?.status === "paid" ? "Paid" : "Pending"}`, margin, y + 18);
    doc.text(`Gateway: ${order.payment?.provider || "Razorpay"}`, margin, y + 33);
    doc.text(`Razorpay Payment ID: ${order.payment?.razorpayPaymentId || "—"}`, margin, y + 48);

    // Summary calculations on the right
    doc.setFont("helvetica", "normal");
    doc.text("Subtotal:", pageWidth - margin - 150, y + 18);
    doc.text(`₹${basePrice.toLocaleString("en-IN")}`, pageWidth - margin - 15, y + 18, { align: "right" });
    doc.text("Integrated GST (18%):", pageWidth - margin - 150, y + 33);
    doc.text(`₹${gst.toLocaleString("en-IN")}`, pageWidth - margin - 15, y + 33, { align: "right" });
    ...
    doc.text(`₹${total.toLocaleString("en-IN")}`, pageWidth - margin - 15, y + 60, { align: "right" });
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    {selectedOrder.payment?.status === "paid" && (
                      <button
                        ...
                        className="mt-3 w-full py-2.5 rounded-lg flex items-center justify-center gap-1.5 text-xs font-bold transition-all active:scale-95 hover:opacity-90"
                        style={{ background: CS.primary, color: CS.onPrimary }}
                      >
                        <span className="material-symbols-outlined text-[15px]">download</span>
              ...
              <form onSubmit={saveProfile} className="space-y-5">
                <div className="flex flex-col sm:flex-row gap-5 items-start">
                  ...
                  <div className="flex flex-col items-center gap-3 flex-shrink-0">
                    <div className="w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold text-[#f0ede4] border-2"
